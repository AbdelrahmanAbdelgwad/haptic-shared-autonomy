...
from haptic.gym.envs.box2d.car_dynamics import Car
from haptic.gym.utils import colorize, seeding, EzPickle
...
BORDER_MIN_COUNT = 4
NUM_TILES_FOR_AVG = (
    5  # The number of tiles before and after to takeinto account for angle
)
...
SHOW_GROUP_INTERSECTIONS = (
    0  # Shows each group of intersections in its own hippie color
)
...
if ZOOM_OUT:
    ZOOM = 0.25  # Complementary to ZOOM_OUT
...
class CarRacing(gym.Env, EzPickle):
    ...
    def check_timeout(self, reward, done):
        if (
            self.t - self.last_touch_with_track > self.max_time_out
            and self.max_time_out > 0.0
        ):
    ...
    def _update_obstacles_info(self):
        different_count = (
            self.obstacle_contacts["count"] != self.obstacle_contacts["count_delay"]
        )
    ...
    def add_current_tile(self, id, lane):
        ...
        if (
            abs((self.track[id, 0, 1] - self.car.hull.angle + np.pi / 2) % (np.pi * 2))
            > np.pi
        ):
            direction = -1
        else:
    ...
    def _remove_prediction(self, id, lane, direction):
        ...
        if len(self._next_nodes) > 0:
            if (
                id in self._next_nodes[0]
                and lane in self._next_nodes[0][id]
    ...
    def _get_next_node(self, id, lane, direction):
        ...
        if (self.info[id]["end"] == True and direction == 1) or (
            ...
            next_id = (id_relative + direction) % len(
                self.tracks[self.info[id]["track"]]
            )  # TODO direction 0
                    ...
                    if self.info[tmp_id]["track"] > 0:
                        if self.info[tmp_id]["end"]:
                            ...
                            if tmp_id != id - id_relative + next_id:
                                direction = 0
                        if any(
                            self.info[list(self._current_nodes.keys())]["track"] > 0
                        ):
            ...
            for k, lanes in deepcopy(list(next_nodes.items())):
                for lane, direction in lanes.items():
                    ...
                    if FORBID_HARD_TURNS_IN_INTERSECTIONS:
                        ...
                        if (
                            direction_tmp - (self.car.hull.angle + np.pi) + np.pi / 4
                        ) % (np.pi * 2) < np.pi / 2:
                            remove = True
                    else:
                        # forbid returning
            ...
            if self.info[node]["track"] == 0:
                ...
                if self.info[node]["end"] or not self.info[node]["start"]:
                    ...
                    next_relative_candidate_safe = (
                        relative_candidate - int(MIN_SEGMENT_LENGHT / 2)
                    ) % len(self.tracks[self.info[node]["track"]])
                    next_relative_candidate = (relative_candidate - 1) % len(
                        self.tracks[self.info[node]["track"]]
                    )
                    candidates.append(
                        node - relative_candidate + next_relative_candidate
                    )
                    secure_candidates.append(
                        node - relative_candidate + next_relative_candidate_safe
                    )
    ...
    def _create_obstacles(self):
        ...
        for inter_tile in np.where(self.info["intersection_id"] != -1)[0]:
            ...
            possible_candidates = [
                item for item in possible_candidates if item not in unvalid_candidates
            ]
        ...
        for count, idx in enumerate(obstacle_tiles_ids):
            ...
            if self.random_obstacle_shape:
                ...
                p1 = (
                    x - width * math.cos(beta) + TRACK_DETAIL_STEP / 2 * math.sin(beta),
                ...
                p1 = (
                    x + TRACK_DETAIL_STEP / 2 * math.sin(beta),
                    y - TRACK_DETAIL_STEP / 2 * math.cos(beta),
                )
                p2 = (
                    x
                    + 2 * width * math.cos(beta)
                    + TRACK_DETAIL_STEP / 2 * math.sin(beta),
                    y
                    + 2 * width * math.sin(beta)
    ...
    def _create_info(self):
        ...
        for i in range(1, len(self.tracks)):
            ...
            info[len(self.tracks[i - 1]) : len(self.tracks[i - 1]) + len(track)][
                "track"
            ] = i  # This wont work for num_tracks > 2
        ...
        if self.num_tracks > 1:
            ...
            for group in groups:
                ...
                for idx in group:
                    d = np.linalg.norm(
                        self.track[track_len:, 1, 2:] - self.track[idx, 1, 2:], axis=1
                    )
                    if d.min() < min_dist:
                        min_dist = d.min()
                        min_dist_idx = idx
        ...
        for num_track in range(self.num_tracks):
            ...
            inters = np.logical_or(
                info[info["track"] == num_track]["t"],
            ...
            while np.abs(angles).max() != 0.0:
                ...
                idxs_safety = [
                    (max_rel_idx + j) % track_len
                    for j in range(-NUM_TILES_FOR_AVG * 2, NUM_TILES_FOR_AVG * 2)
                ]
        ...
        info["angle"][max_idxs] = (
            self.track[max_idxs, 0, 1] - self.track[max_idxs, 1, 1]
        )
    ...
    def _set_lanes(self):
        if self.num_lanes_changes > 0 and self.num_lanes > 1:
            ...
            changes = np.sort(
                self.np_random.randint(0, len(self.track), self.num_lanes_changes)
            )
            ...
            for pos, idx in enumerate(changes):
                ...
                changes_in_track = changes_in_track[
                    (changes_in_track < until) * (changes_in_track > 0)
                ]
                idx_relative = idx - start_from

                if (
                    sum(
                        ((changes_in_track - idx) > 0) * ((changes_in_track - idx) < 10)
                    )
                    > 0
                ):  # TODO wont work when at end of track
                ...
                for i in range(50 + 1):
                    if (
                        track_info[(idx_relative + i) % len(track_info)]["end"]
                        or track_info[idx_relative - i]["start"]
                    ):
                        changes_bad.append(idx)
                        break
            ...
            for i, point in enumerate(self.track):
                ...
                if (
                    change and rm_lane == 1
                ):  # if it is time to change and the turn is to remove lane
                ...
                if (
                    self.info[i]["end"]
                    or self.info[i]["start"]
                    or counter > self.max_single_lane
                ):
                    rm_lane = 0
            ...
            for num_track in range(self.num_tracks):
                for lane in range(self.num_lanes):
                    for i in range(10):
                        ...
                        self.info[self.info["track"] == num_track][+i]["lanes"][
                            lane
                        ] = True
                        self.info[self.info["track"] == num_track][-i]["lanes"][
                            lane
    ...
    def _generate_track(self):
        if self.load_tracks_from is not None:
            ...
            try:
                dic = pickle.load(
                    open(self.load_tracks_from + "/" + str(idx) + ".pkl", "rb")
                )
            except Exception as e:
                print("######## Error ########")
                ...
                self.info = dic["info"]
                self.obstacle_contacts = np.zeros(
                    (len(self.obstacles_poly)),
                    dtype=[("count", int), ("count_delay", int), ("visited", bool)],
                )
            ...
            if self.info[0]["intersection_id"] != -1:
                return False
            ...
            for j in range(self.num_tracks):
                ...
                for i in range(len(track)):
                    ...
                    if border[i]:
                        ...
                        if self.num_lanes > 1:
                            if side == -1 and self.info[pos]["lanes"][0] == False:
                                c = 0
                            if side == +1 and self.info[pos]["lanes"][1] == False:
                                c = 0

                        b1_l = (
                            x1 + side * TRACK_WIDTH * c * math.cos(beta1),
                            y1 + side * TRACK_WIDTH * c * math.sin(beta1),
                        )
                        b1_r = (
                            x1 + side * (TRACK_WIDTH * c + BORDER) * math.cos(beta1),
                            y1 + side * (TRACK_WIDTH * c + BORDER) * math.sin(beta1),
                        )
                        b2_l = (
                            x2 + side * TRACK_WIDTH * c * math.cos(beta2),
                            y2 + side * TRACK_WIDTH * c * math.sin(beta2),
                        )
                        b2_r = (
                            x2 + side * (TRACK_WIDTH * c + BORDER) * math.cos(beta2),
            ...
            for lane in range(self.num_lanes):
                if self.info[j]["lanes"][lane]:
                    ...
                    if self.info[j]["end"] == False and self.info[j]["start"] == False:
                        ...
                        info_track = self.info[
                            self.info["track"] == self.info[j]["track"]
                        ]
                        j_relative = j - sum(self.info["track"] < self.info[j]["track"])

                        if (
                            info_track[j_relative - 1]["track"]
                            == info_track[j_relative]["track"]
                        ):
                            # If last tile didnt exist
                            if info_track[j_relative - 1]["lanes"][lane] == False:
                                first = True
                        if (
                            info_track[(j_relative + 1) % len(info_track)]["track"]
                            == info_track[j_relative]["track"]
                        ):
                            # If last tile didnt exist
                            if (
                                info_track[(j_relative + 1) % len(info_track)]["lanes"][
                                    lane
                                ]
                                == False
                            ):
                    ...
                    if self.info[j]["end"] == True or self.info[j]["start"] == True:
                        ...
                        for i in [0, 1]:  # because there are two point to do
                            ...
                            if len(p3) == 0:
                                max_idx = sum(
                                    sum(self.info[self.info["track"] == 0]["lanes"], [])
                                ...
                                not_too_close = np.where(
                                    np.linalg.norm(np.subtract(p3_org, p1), axis=1)
                                    >= TRACK_WIDTH / 3
                                )[0]
                                while len(p3) == 0 and distance < PLAYFIELD:
                                    ...
                                    p3 = [
                                        p3_org[i]
                                        for i in np.intersect1d(close, not_too_close)
                                    ]
                            ...
                            d = (
                                np.cross(np.subtract(p2, p1), np.subtract(p1, p3))
                            ) ** 2 / np.linalg.norm(np.subtract(p2, p1))
    ...
    def _transform_action(self, action):
        ...
        if self.discretize_actions == "soft":
            ...
            if action == 1:
                action = [-1, 0, 0.0]  # Left
            if action == 2:
                action = [+1, 0, 0.0]  # Right
            if action == 3:
                action = [0, +1, 0.0]  # Accelerate
            if action == 4:
                action = [0, 0, 0.8]  # break
        ...
        if self.num_tracks > 1:
            ...
            inter2 = np.array(
                [
                    ...
                    for x in points2
                    if (
                        np.linalg.norm(points1[:, 1, :] - x[1:], axis=1)
                        <= TRACK_WIDTH / 3.5
                    ).sum()
                    >= 1
                ]
            ...
            for i in range(inter2.shape[0]):
                if (
                    np.array_equal(inter2[i - 1, 1, :], inter2[i, 0, :]) == False
                    or np.array_equal(
                        inter2[i, 1, :], inter2[((i + 1) % len(inter2)), 0, :]
                    )
                    == False
                ):
                ...
                if sec1 is not False and sec2 is not False:
                    ...
                    if remove is False:
                        ...
                        # if there are more than 50 tiles very close to main track
                            ...
                            len(min_distances) > 50
                            and (min_distances < TRACK_WIDTH * 2).sum() > 50
                        ):
                            remove = True
                    ...
                    if remove:
                        ...
                        key = np.where(
                            np.all(track1[:, :, [2, 3]] == sec1[0], axis=(1, 2))
                        )[0]
                        val = np.where(
                            np.all(track2[:, :, [2, 3]] == sec2[0], axis=(1, 2))
                        )[0] + len(track1)
                        intersection_keys.append(key[0])
                        intersection_vals.append(val[0])

                        key = np.where(
                            np.all(track1[:, :, [2, 3]] == sec1[-1], axis=(1, 2))
                        )[0]
                        val = np.where(
                            np.all(track2[:, :, [2, 3]] == sec2[-1], axis=(1, 2))
                        )[0] + len(track1)
                        intersection_keys.append(key[0])
                        intersection_vals.append(val[0])

            track2 = np.delete(
                track2, list(removed_idx), axis=0
            )  # efficient way to delete them from np.array
    ...
    def _render_tiles(self):
        ...
        for poly, color, id, lane in self.road_poly:
            if (
                SHOW_NEXT_N_TILES > 0
                and id in self._trail_nodes
                and lane in self._trail_nodes[id]
            ):
    ...
    def render_debug_clues(self):
        ...
        if SHOW_TURNS:
            for x, y in self.track[np.abs(self.info["angle"]).argsort()[-10:]][
                :, 1, 2:
            ]:
                gl.glColor4f(1, 0, 0, 1)
                gl.glVertex3f(x + 1, y + 1, 0)
    ...
    def _get_rnd_position_inside_lane(
        self, idx, border=True, direction=1, discrete=False
    ):
        """
        idx of tile
    ...
    def _get_position_inside_lane(
        self, idx, x_pos, border=True, direction=1, discrete=False
    ):
        """
        ...
        if discrete:
            ...
            x_pos = from_val * x_pos + (1 - x_pos) * (
                to_val - TRACK_WIDTH * (1 - border)
            )
        else:
    ...
    def _get_tile_near_tile_in_filter(self, filter, tiles_before=8):
        ...
        idx_general = (idx_relative + tiles_before) % len(track) + (
            self.info["track"] < self.info[idx]["track"]
        ).sum()
...
def play(env):
    ...
    def key_release(k, mod):
        ...
        if k == key.Z:
            env.change_zoom()
        if k == key.G:
            env.switch_intersection_groups()
        if k == key.I:
            env.switch_intersection_points()
        if k == key.X:
            env.switch_xt_intersections()
        if k == key.E:
            env.switch_end_of_track()
        if k == key.S:
            env.switch_start_of_track()
        if k == key.T:
            env.screenshot("./")
        if k == key.Q:
            sys.exit()
